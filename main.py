...
googleLog.log("Starting to log into sheets",
              sheetName="logs",
              tabName="bookingForm")
...
def add_timestamp_to_filename(filename):
    timestamp = datetime.now().strftime(
        "%y%m%d%H%M%S%f")[:-4]  # Remove last 4 digits of microseconds
...
def extract_data_from_image(filename):
    try:
        ...
        print(
            f"extract_data_from_image base64_image: base64_image too big to print"
        )
                ...
                "content": [
                    {
                        ...
                        "text",
                        'text':
                        "Convert the image file into a JSON string. extract the data from the attached form into Json data. Use the left hand column as the key for the data itself. If the data is blank insert value 'none' as data. If a date field is missing the year, put the current year into that date. Where the data is a date, convert to british numeric format ie dd/mm/yyyy. For 'deposit paid' and 'balance due', return just a float without any currencies. For Lead Customer and Customer 2,3,4 return as nested JSON with: title, firstName, middleName, lastName, dob. For address, return as nested JSON with: postcode, city, addressLine1, addressLine2 etc.. For Airport From / To - return both IATA and full airport name along with the time and flight number. Return just the json data, no preamble so the first character is '{'."
                    },
                    {
                        "type": "image_url",
                        "image_url": {
    ...
    if request.method == "POST":
        ...
        if not valid:
            return render_template(
                ...
                uploaded=None,
                json_data=None,
                error="Invalid file type. Only JPEG and PDF are allowed.")
        ...
        loggerPerm.log(
            f"Stored file path in Replit DB file_path:{file_path}, file.filename: {workingFilename} "
        )
        ...
        if file.filename.lower().endswith(".pdf"):
            ...
            filePDF_path = os.path.join(app.config['STORAGE_DIR'],
                                        workingFilename)
            ...
            print(
                f"/home filePDF_path:{filePDF_path}, filenameJPG: {filenameJPG}, fileJPG_path:{fileJPG_path}, outputFolder:{outputFolder}"
            )

            print(
                f"/home about to convert_from_path with filePDF_path:{filePDF_path} to filenameJPG: {filenameJPG}"
            )
            fileJPG = convert_from_path(filePDF_path,
            ...
            print(
                f"Just converted with fileJPG:{fileJPG}, {dir(fileJPG)}, genfiles:{generated_files}"
            )
            ...
            for filename in generated_files:
                ...
                if filename not in existing_files:  # ✅ Check if the file is already in Replit DB
                    ...
                    print(
                        f"⚠️ {filename} already exists in Replit DB, skipping."
                    )

        loggerPerm.log(
            f"About to exit /home, with workingFilename:{workingFilename}")
        print(
            f"About to exit /home, with workingFilename:{workingFilename}, displayFilename:{displayFilename}"
        )
        return render_template(
            "home.html",
            uploaded=f"File '{displayFilename}' uploaded successfully.",
            json_data=None)
    return render_template("home.html", uploaded=None, json_data=None)
...
def extract_booking_data():
    ...
    if not uploaded_files:
        return render_template("home.html",
                               uploaded="No uploaded files found!",
                               json_data=None)

    file_path = os.path.join(app.config['STORAGE_DIR'], str(workingFilename))
    loggerPerm.log(
        f"/extract Processing file: {workingFilename}, path:{file_path}")
    ...
    return render_template(
        "home.html",
        uploaded=f"File '{workingFilename}' uploaded successfully!",
        json_data=jsonDict)
...
def downloadToLocal(filename):
    ...
    print(
        f"downloadToLocal called using STORAGE_DIR:{STORAGE_DIR}, filename:{filename}"
    )
    if filename in db:
...
def delete_file(filename):
    ...
    if filename in db:
        ...
        if os.path.exists(file_path):
            ...
            return jsonify({
                "error":
                "❌ File not found on disk, reference removed from DB"
            }), 404
    else:
...
def upload_file():
    ...
    try:
        ...
        if not valid:
            ...
            return jsonify(
                {"error":
                 "Invalid file type. Only JPEG and PDF are allowed."}), 400
        ...
        if file.filename.lower().endswith(".pdf"):
            try:
                ...
                name, ext = workingFilename.rsplit(".", 1)
                filenameJPG = f"{name}.jpg"
                fileJPG_path = os.path.join(app.config['STORAGE_DIR'],
                                            filenameJPG)
                outputFolder = os.path.join(app.config['STORAGE_DIR'])

                loggerPerm.log(
                    f"File paths: PDF={filePDF_path}, JPG={fileJPG_path}, OutputFolder={outputFolder}"
                )
                ...
                generated_files = [
                    ...
                    for f in os.listdir(outputFolder) if f.endswith(".jpg")
                ]

                if not fileJPG:
                    loggerPerm.log(
                        f"ERROR: PDF {workingFilename} conversion failed, no images generated."
                    )
                ...
                loggerPerm.log(
                    f"ERROR: PDF to JPG conversion failed - {str(e)}\n{traceback.format_exc()}"
                )
        ...
        try:
            ...
            jsonStrConcat = re.search(r'\{.*\}', extracted_data, re.DOTALL)
            jsonDict = json.loads(
                jsonStrConcat.group(0)) if jsonStrConcat else {}

            loggerPerm.log(f"Extraction complete for {workingFilename}")
        except Exception as e:
            loggerPerm.log(
                f"ERROR: Data extraction failed - {str(e)}\n{traceback.format_exc()}"
            )
        ...
        loggerPerm.log(
            f"ERROR: Unexpected server error - {str(e)}\n{traceback.format_exc()}"
        )
